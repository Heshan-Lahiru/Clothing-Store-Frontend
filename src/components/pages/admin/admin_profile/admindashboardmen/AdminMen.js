...
export default function AdminMen() {
  ...
  const fetchData = () => {
    axios.get('http://localhost:9070/getadminmencloths')
      .then(response => {
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:9070/deletemencloth/${id}`)
      .then(() => {
        fetchData();
      })
      .catch(error => {
...
const handleChange = (e) => {
 ...
 const handleFileChange = (e) => {
  ...
  const updateform = () => {
    const { menid, name, type, price, qty, description, isactive, image } = updatedata;
    ...
    formData.append(
      'ClothDao',
      JSON.stringify({ menid, name, type, price, qty, description, isactive })
    );
    ...
    axios
      .put('http://localhost:9070/menclothupdate', formData, {
        headers: {
      ...
      <div className="row g-0 min-vh-100">
        ...
        >
          <div className="p-3 p-lg-5">
            <h1 className="mb-4">Men's Clothing</h1>
                ...
                <tbody>
                  {data.map((item, index) => (
                    <tr key={index}>
                      <td>
                        <img 
                          src={`http://localhost:9070/images/men/${item.image}`}
                          alt={item.name}
                          className="rounded"
                      ...
                      <td>
                        <div className="d-flex gap-2">
                          <button 
                            ...
                            onClick={() => handleDelete(item.menid)}
                          >
                            Delete
                          </button>
                          <button 
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div className="mb-4">
                ...
                {imagePreview && (
                  <img 
                    src={imagePreview.startsWith("blob:") ? imagePreview : `http://localhost:9070/images/men/${imagePreview}`} 
                    alt="Selected" 
