...
export default function AdminWomen() {
  ...
  const fetchData = () => {
    axios.get('http://localhost:9070/getadminwomencloths')
      .then(response => {
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:9070/deletewomencloth/${id}`)
      .then(() => {
        fetchData();
      })
      .catch(error => {
      ...
      <div className="row g-0 min-vh-100">
        ...
        >
          <div className="p-3 p-lg-5">
            <h1 className="mb-4">Women's Clothing</h1>
                ...
                <tbody>
                  {data.map((item, index) => (
                    <tr key={index}>
                      <td>
                        <img 
                          src={`http://localhost:9070/images/women/${item.image}`}
                          alt={item.name}
                          className="rounded"
                      ...
                      <td>
                        <div className="d-flex gap-2">
                          <button 
                            ...
                            onClick={() => handleDelete(item.womenid)}
                          >
                            Delete
                          </button>
                          <button 
